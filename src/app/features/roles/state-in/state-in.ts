...
@Component({
  ...
  templateUrl: './state-in.html',
  styleUrl: './state-in.scss',
})
export class StateIn implements OnInit {  
  ...
  loadPermissions() {
    this.loading = true;
    this.api.getStateInByRole(this.roleId).subscribe({
  ...
  savePermissions() {
    ...
    this.api.updateRoleStateIn( payload).subscribe({
      next: (res) => {
        this.saving = false;
